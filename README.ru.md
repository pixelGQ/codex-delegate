...
## Зачем
- Основной Codex может делегировать второму Codex: агент ищет в интернете, пишет код, гоняет команды, пока основная сессия остаётся чистой.
- Разные политики: отдельный токен, песочница и сетевые настройки — рисковый/шумный ворк не трогает основную учётку.
- Прозрачность: стримы, JSONL, логи и TUI легко расширить до веб/desktop UI.

## Типовые сценарии
- **Ресерч на фоне**: `delegate "собери свежие новости про <тему>" --research --live` — агент ищет, выдаёт 3–5 источников, вы работаете дальше.
- **Долгие команды**: билды/тесты в нужном репо через `--cwd`, а вывод видно в TUI.
- **Код с контекстом**: `--context src/file1.ts,README.md` — агент получает нужные фрагменты и пишет правки точнее.
- **Несколько задач параллельно**: ставите в очередь, в TUI видны статусы, длительность, логи.
- **Сохранить результат**: `--artifact reports/summary.txt` — stdout уходит в файл (относительно `--cwd`).
